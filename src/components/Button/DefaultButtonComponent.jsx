 ...
 * @param {string} roundedOff - roundedOff the button.
 ...
 */
const DefaultButtonComponent = ({ title, onClick, className, isDisabled, icon, iconBefore, color, roundedOff }) => {
  return (
    // Button with dynamic classes based on provided props
    <button
      disabled={isDisabled}
      type="button"
      className={` ${color ? "" : "btn btn-primary"}  ${className} ${!roundedOff && 'rounded-2xl'} `}
      onClick={onClick}
    >
